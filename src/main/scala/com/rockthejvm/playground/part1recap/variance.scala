...
object variance {
  ...
  val myVet: Vet[Dog] = new Vet[Animal]{
    override def heal(animal: Animal) = {
      println("Here you go , your're good now...")
      true
    }
  }
  ...
  - if the type CONSUMES or ACTS ON values of type A (e.g. a vet, serializers, processors), then the type should be CONTRAVARIANT
  - otherwise, INVARIANT
  */
  ...
  */
  ...
  class Vet2[-A](val favoriteAnimal: A) <-- the type of val field are in COVARIANT position
 ...
 abstract class Vet2[-A] {
  ...
  val rescueDog: Dog =n lassieVet.rescueAnimal() // must return a Dog, but it returns a Cat - type conflict!
